...
class ApplyOperator(AbstractVariationalDriver):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if variational_state is None:
            variational_state = copy.copy(original_state)

        if variational_state.hilbert != operator.hilbert or original_state.hilbert != operator.hilbert:
            raise TypeError(
                dedent(
                    ...
                    the operator has hilbert space {operator.hilbert}, and the original state has {original_state.hilbert}.
                    The three should match.
                    """
                )
            )
        ...
        super().__init__(variational_state, optimizer, minimized_quantity_name="Mean Squared Error")

        self._operator = operator.collect()  # type: AbstractOperator
        self._origState = original_state;
